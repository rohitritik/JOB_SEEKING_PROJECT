...
import { Application } from "../models/applicaionSchema.js";
import cloudinary from "cloudinary";
import { Job } from "../models/jobSchema.js"

export const employerGetAllApplications = catchAsynError(async (req, res, next)=>{
    const {role} = req.user;
    if(role === "Job Seeker"){
        return next(
...
export const jobSeekerGetAllApplications = catchAsynError(async (req, res, next) => {
    const { role, _id } = req.user;  // Destructure role and _id from the user object
    ...
    // Find all applications for the logged-in Job Seeker
    const applications = await Application.find({ 'applicantID.user': _id });

    // If no applications are found, respond with a message
...
export const jobSeekerDeleteApplications = catchAsynError(async (req, res, next)=>{
    const {role} = req.user;
    if(role === "Employer"){
        return next(
...
export const postApplication = catchAsynError(async(req,res,next)=>{
    const {role} = req.user;
    if(role === "Employer"){
        return next(
            new ErrorHandler("Employer is not allowed to access this resources!", 400)
